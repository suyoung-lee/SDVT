...
def evaluate_ml10(args,
    ...
    for episode_idx in range(num_episodes):
        ...
        for step_idx in range(num_steps):
            ...
            # observe reward and next obs
            [state, belief, task], (rew_raw, rew_normalised), done, infos = utl.env_step(envs, action, args)
            #print(episode_idx, step_idx, state)
            done_mdp = [info['done_mdp'] for info in infos]
...
def visualise_behaviour(args,
    ...
    if hasattr(unwrapped_env, 'visualise_behaviour'):
    ...
    elif hasattr(unwrapped_env, 'visualise_behaviour_ml10'):
        # if possible, get it from the env directly
        # (this might visualise other things in addition)
        traj = unwrapped_env.visualise_behaviour_ml10(env=env,
                                                 args=args,
                                                 policy=policy,
                                                 iter_idx=iter_idx,
                                                 encoder=encoder,
        ...
        latent_means, latent_logvars, episode_prev_obs, episode_next_obs, episode_actions, episode_rewards, episode_returns, success = traj
    else:
        traj = get_test_rollout(args, env, policy, encoder)
        latent_means, latent_logvars, episode_prev_obs, episode_next_obs, episode_actions, episode_rewards, episode_returns = traj

    if latent_means is not None and not hasattr(unwrapped_env, 'visualise_behaviour_ml10'):
    ...
    if success is None:
        ...
        return torch.stack(episode_returns).sum(dim=0).cpu().numpy()[0][0], success
