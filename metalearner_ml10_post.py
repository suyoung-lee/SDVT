        ...
        while self.iter_idx < self.num_updates:
            ...
            with torch.no_grad():
                if self.args.vae_mixture_num>1:
                    ...
                    if virtual and self.iter_idx%self.args.max_rollouts_per_task==0:
            ...
            for step in range(self.args.policy_num_steps):
                ...
                if virtual: #use virtual environment
                    with torch.no_grad():
                        ### state generation
                        if step == 0 and self.virtual_iter_idx % self.args.max_rollouts_per_task!= 0:
                            next_state_pred = virtual_init_state.clone()
                        else:
                            next_state_pred = self.vae.state_decoder(latent_sample_v.detach(), prev_state, action)
                        ...
                        if self.args.virtual_intrinsic > 0.0:
                            rew_int = F.cosine_similarity(y,y_intercept.detach().clone()).unsqueeze(dim=1)
                            rew_raw = torch.clamp(rew_raw_pred.clone().detach() + self.args.virtual_intrinsic * rew_int.clone(), min=0.0, max=10.0)  # add intrinsic reawrd
                        else:
                ...
                #print("=" * 10, self.iter_idx, self.virtual_iter_idx, step, "=" * 10, virtual, y_intercept_)
