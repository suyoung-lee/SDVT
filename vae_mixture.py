...
from models.decoder_mixture import StateTransitionDecoder_mixture, RewardDecoder_mixture, TaskDecoder_mixture
from models.encoder_mixture import RNNEncoder_mixture
...
class VaribadVAEMixture:
    ...
    def initialise_decoder(self):
        ...
        if self.args.decode_state:
            state_decoder = StateTransitionDecoder_mixture(
                args=self.args,
        ...
        if self.args.decode_reward:
            reward_decoder = RewardDecoder_mixture(
                args=self.args,
        ...
        if self.args.decode_task:
            ...
            task_decoder = TaskDecoder_mixture(
                class_dim=self.args.vae_mixture_num,
                latent_dim=latent_dim,
    ...
    def compute_rew_reconstruction_loss(self, latent, prev_obs, next_obs, action, reward, return_predictions=False, y=None):
        ...
        if self.args.multihead_for_reward: #not tested for mixture yet
            ...
            if y is None:
                ...
                rew_pred, y_mu, y_var = self.reward_decoder(latent, next_obs, prev_obs, action.float(), y)
        ...
        if return_predictions:
            if y is None:
                ...
                return loss_rew, rew_pred, y_mu, y_var

        else:
            if y is None:
                return loss_rew
            else:
                return loss_rew, y_mu, y_var
    ...
    def compute_loss(self, latent_mean, latent_logvar, vae_prev_obs, vae_next_obs, vae_actions,
        ...
        if self.args.decode_reward:
            ...
            rew_reconstruction_loss, y_mu, y_var = self.compute_rew_reconstruction_loss(dec_embedding, dec_prev_obs, dec_next_obs,
