        ...
        while self.iter_idx < self.num_updates:
            ...
            for step in range(self.args.policy_num_steps):
                ...
                if virtual: #use virtual environment
                    #print(self.iter_idx, step, 'rew_raw: ', rew_raw)
                    with torch.no_grad():
                        rew_raw_pred = self.vae.reward_decoder(latent_sample_v.detach(), next_state, prev_state, action)
                        #rew_raw_pred = torch.clamp(2.0 * rew_raw_pred, min=0.0, max=10.0)
                    ...
                    #print('rew_raw_pred: ', rew_raw_pred, '\n rew_normalised:', rew_normalised)
                    #print('latent_sample: ', latent_sample, '\n latent_sample_v:', latent_sample_v)
