...
from utils import evaluation_extensive as utl_eval
...
class MetaEvalML10:
    ...
    def __init__(self, args):
        ...
        self.num_updates = int(self.args.num_frames) // self.args.policy_num_steps // self.args.num_processes
        self.iter_idx = int(self.args.load_iter)
        self.frames = (self.iter_idx+1)*self.args.policy_num_steps*self.args.num_processes
        ...
        self.envs = make_vec_envs(env_name=self.args.env_name, seed=self.args.seed, num_processes=self.args.num_processes,
    ...
    def log(self):
        ...
        save_episode_successes = (self.iter_idx + 1) % (20 * self.args.eval_interval) == 0
        if save_episode_successes:
            episode_successes_array = np.zeros((15, total_parametric_num, self.args.max_rollouts_per_task, self.envs._max_episode_steps))
        ...
        for task_class in [14, 12]:
            for parametric_num in range(total_parametric_num//num_worker):
                print("task_class {}, parametric_num {}".format(task_class, parametric_num))
                ...
                returns_per_episode, latent_mean, latent_logvar, successes, prob, episode_probs, episode_successes = utl_eval.evaluate_ml10(args=self.args,
                                                        policy=self.policy,
                                                        ret_rms=ret_rms,
        ...
        with open(self.logger.full_output_folder + '/log_eval.csv', 'a', encoding='UTF8') as f:
            writer = csv.writer(f)
            writer.writerow(np.concatenate(([self.iter_idx, int(self.frames)], taskwise_mean_return, taskwise_mean_success)))
        ...
        if save_episode_successes:
            np.save('{}/{}/episode_successes_array.npy'.format(self.logger.full_output_folder, self.iter_idx), episode_successes_array)
        if self.args.vae_mixture_num > 1:
