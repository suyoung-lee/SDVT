...
class ML10TEST(Env):
    ...
    def reset_task(self, task=None):
        if task is None:
            env_ind = random.choice(range(5))
            self._env = self.test_env_cls_list[env_ind]
            _env_name = self.test_env_name_list[env_ind]
            subtask_ind = random.choice(range(50))
            self._task = [env_ind, subtask_ind]
    ...
    def visualise_behaviour_ml10(env,
        ...
        if image_folder is not None:
            behaviour_dir = '{}/{}/{:01d}_{:02d}'.format(image_folder, iter_idx, task_num[0],task_num[1])
