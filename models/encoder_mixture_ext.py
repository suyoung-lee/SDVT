...
class RNNEncoder_mixture_ext(nn.Module):
    def __init__(self,
                 args,
                 # network size
                 layers_before_gru=(),
        ...
        super(RNNEncoder_mixture_ext, self).__init__()
    ...
    def forward(self, actions, states, rewards, hidden_state, return_prior, sample=True, detach_every=None, y_intercept=None):
        ...
        if y_intercept is not None: # ignore the previous steps and just cut through graph
            # q(z|x,y)
            mu, var, z = self.qzxy(gru_h_reshaped, y_intercept) #procsx5 or (5000batchsize)xlatent_dim
        else:
